...
module('Integration | Component | question-item', function (hooks) {
  ...
  test('test quetion input options', async function (assert) {
    ...
    assert.equal(
      questEl.querySelector('[data-test-question-item-title]').textContent,
      multiChoice.headline,
      `Title should show question.headline value`
    );
    ...
    assert.equal(
      questEl.querySelector('[data-test-question-item-desc]').textContent,
      'Hello world',
      `Description should show the description of the question when question.description is not null`
    );
    ...
    assert.equal(
      ...
      `Description should show 'Wählen Sie alles Zutreffende aus' when a muitiple select question has null question.description`
    );
    ...
    assert.equal(
      ...
      `Description should show 'Wählen Sie eine Antwort aus' when a single select question has null question.description`
    );

    // expecting  number of checkboxes equaling multiChoice.choices.length
    this.set('quest', { ...multiChoice, multiple: true });
    assert.equal(
      questEl.querySelectorAll('input[type=checkbox]').length,
      multiChoice.choices.length,
      `expecting ${multiChoice.choices.length} checkboxes when question is multichoice and multiple select`
    );

    // expecting  number of radio buttons equaling multiChoice.choices.length
    this.set('quest', { ...multiChoice, multiple: false });
    assert.equal(
      questEl.querySelectorAll('input[type=radio]').length,
      multiChoice.choices.length,
      `expecting ${multiChoice.choices.length} radio buttons when question is multichoice and multiple is false`
    );

    // expecting  an input when question_type is text and multiline is false
    this.set('quest', { ...textQuestion, multiline: false });
    assert.equal(
      questEl.querySelectorAll('input').length,
      1,
      `expecting 1 input box when question_type=text and multiline is false`
    );

    // expecting  a textarea when question_type is text and multiline is true
    this.set('quest', { ...textQuestion, multiline: true });
    assert.equal(
      questEl.querySelectorAll('textarea').length,
      1,
      `expecting 1 textarea when question_type=text and multiline is true`
    );
  });
});
