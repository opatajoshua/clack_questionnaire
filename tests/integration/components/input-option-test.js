...
module('Integration | Component | input-option', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.equal(
      inputEl.querySelectorAll('input').length,
      1,
      `expecting a input box when question_type=text and multiline is false`
    );

    // expecting  a textarea when question_type is text and multiline is true
    this.set('quest', { ...textQuestion, multiline: true });
    assert.equal(
      inputEl.querySelectorAll('textarea').length,
      1,
      `expecting a textarea when question_type=text and multiline is true`
    );
  });
});
